...
{
  prob = approx(enfVec,enfDet,enfEffort)$y #we would use spline --> marginal impact
  return(prob)
}
...
fineExp = function(enfEffort,fine,Q,Qstar) #raw fine amount
{
  if (Q>Qstar) {
...
#  if (Q>Qstar) taxRev = fineProbFunc(enfEffort) * (Q-Qstar)* tax + Qstar*tax else taxRev = tax*price*Q
  taxRev = min(tax*price*Qstar,tax*price*Q) #not relevant for bioeconomic model
...
if (archetype ==1) { #most similar to what we are doing in our model; we will use fineExp func and costEnforcementFunc
...
} #Close Run DynamicOptEnf function
